...
The graphical builder will:

1. Use the installed Python environment.
2. Install or update the required Python packages.
3. Build `OCC Inventory Manager.exe` directly in the project folder.
4. Locate the Inno Setup compiler.
5. Compile `OCC_Inventory_Manager_Setup.exe`.
6. Save the installer in the folder selected in the build window.

The finished application executable remains in the project folder. The installer is ready for distribution to end users.

## Build the installer

1. Install Inno Setup.
2. Build the app with `build.bat`.
3. Open `OCC_Inventory_Manager.iss`.
4. Click **Compile**.
...
If the build does not start or fails, review `build_error.log` in the project folder. The graphical builder now also displays callback and worker-thread errors instead of failing silently.

The builder does not create a separate virtual environment.
